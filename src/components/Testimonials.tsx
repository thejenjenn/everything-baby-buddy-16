      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 animate-fade-in" style={{animationDelay: "0.3s"}}>
          {testimonials.map((testimonial, index) => (
            <Card key={index} className="border-2 border-gray-200 transition-all duration-300 bg-white/80 backdrop-blur-sm rounded-2xl animate-scale-in" style={{animationDelay: `${index * 0.1}s`, boxShadow: '0 4px 16px rgba(0, 0, 0, 0.08)'}}>
              <CardContent className="p-8 text-center">
                ...
                <div className="flex justify-center mb-4">
                  {[...Array(testimonial.rating)].map((_, i) => (
                    <Star key={i} className="h-4 w-4" style={{color: "#FFC107", fill: "#FFC107"}} />
                  ))}
                </div>
